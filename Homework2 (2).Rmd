...
pizza <- read.csv("/Users/jennifercenta/Downloads/363/Pizza.csv")
...
*In analyzing the Pizza dataset, non-normal distributions and clustering by brand posed challenges for traditional statistical assumptions. Despite this, Principal Component Analysis (PCA) was a valuable tool for dimensionality reduction. Our findings, supported by evidence from scatterplots, revealed distinct clusters of pizza samples in the score plot, emphasizing the effectiveness of PCA in capturing underlying patterns. Multivariate outliers were identified, particularly outside the 95% Confidence Ellipse, offering insights into unique attributes to the pizza samples. The sample size of 300 for 7 variables is sufficient for PCA as it exceeds the N = 2p rule for Chi-square quantile plots. The bi-plot further demonstrated relationships between variables and brands in the reduced-dimensional space. Overall, PCA simplified the data to enable interpretation of principal components, and facilitated the identification of nutritional characteristics associated with different pizza brands, despite the non-normality and clustering challenges.*
