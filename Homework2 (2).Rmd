...
```
*We have chosen to retain Components One and Two. Component One has positive association with moisture, protein, log-transformed fat, ash, log-transformed sodium, and log-transformed calories, and a negative association with carbs. More weight is held on protein, log fat, ash, and sodium (all positive association) and carbohydrates (negative association). Component One captures the overall nutritional richness and composition of pizzas. Meanwhile, Component Two has positive association with moisture, protein, and ash, and negative association with log fat, log sodium, carb, and logcal. The most weight is held on moisture (positive) and calories (negative). Component Two captures a potential nutritional pattern where pizzas are characterized by being moist, but at the same time, tend to be lower in overall calories.*
...
#make sure all colors are visible
library(RColorBrewer)
palette(brewer.pal(n = 10, name = "Set3"))

#label by brand type
text(pc1$scores[, 1], pc1$scores[, 2], labels = pizzatrans[, 1], cex = 0.5, col = (as.numeric(factor(pizzatrans[, 1]))))
...
```
*The score plot for the pair of Component One and Two shows clustering of data points. By looking at the labels of each pizza brand, it is clear that most brands form clusters of data points within the Confidence Ellipse, which suggests that certain brands of pizzas share common nutritional features within their products, which could be related to factors such as ingredients or cooking methods. There is a cluster of outliers that are outside the 95% Confidence Ellipse which suggests that there are data points that deviate significantly from the general pattern captured by the principal components. These outliers represent unique or unusual pizza samples within brand A which contain distinct nutritional profiles compared to the majority of the dataset and other brands. Looking at the bi-plot also shows the distinct clusters, and where they lie relative to the variables.*

### Question 6

*Overall through PCA we were able to reduce 7 components into 2 principal components, the first component influeced mainly by protein, log fat, ash, sodium, and carbohydrates, and the second component influenced mainly by moisture and calories of the pizza samples. In analyzing the Pizza dataset, clustering by brand posed challenges for traditional statistical assumptions such as checking for linearity in p-dimensional space through scatterplots. Despite this, Principal Component Analysis (PCA) was a valuable tool for dimensionality reduction due to the high correlations between variables found in our correlation matrix. Our findings revealed distinct clusters of pizza samples in the score plot, emphasizing the effectiveness of PCA in capturing underlying patterns. However, it should be noted that multivariate outliers were identified outside the 95% Confidence Ellipse, in particular for brand A pizza samples. The sample size of 300 for 7 variables is sufficient for PCA as it exceeds the N = 2p rule for Chi-square quantile plots. The bi-plot further demonstrated relationships between variables and brands in the reduced-dimensional space.*
